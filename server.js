...
function publicState(viewerId = "") {
  ...
  return {
    ...
    revealedAnswer: game.revealed && challenge ? challenge.name : null,
    winnerAnnouncement: game.winnerAnnouncement,
    winnerAnnouncementId: game.winnerAnnouncementId,
    ...
    challenges: challenges
      .filter(({ id }) => !game.usedChallengeIds.includes(id))
      .map(({ id, continent, name }) => ({ id, continent, name })),
    players: onlinePlayers().map(({ id, name, score, online, roundAttempts = 0, roundCorrect = false }) => ({
      ...
      correctGuess: id === viewerId ? players.get(id)?.correctGuess || "" : "",
      online,
      isHost: id === hostId
    }))
  };
...
function startRound(playerId, challengeId) {
  ...
  if (game.usedChallengeIds.includes(challengeId)) {
    return { ok: false, error: "That flag was already used. Choose a new flag." };
  }
  game.round += 1;
  game.challengeId = challengeId;
  game.usedChallengeIds.push(challengeId);
...
function revealAnswer(playerId) {
  ensureHost();
  if (playerId !== hostId) return { ok: false, error: "Only the host can reveal the answer." };
  if (!currentChallenge()) return { ok: false, error: "Start a round before revealing the answer." };
  game.revealed = true;
  return { ok: true };
}
...
function announceWinner(playerId) {
  ensureHost();
  if (playerId !== hostId) return { ok: false, error: "Only the host can announce the winner." };
  const winner = onlinePlayers().find(player => player.id !== hostId);
  if (!winner) return { ok: false, error: "No players to announce yet." };
  game.winnerAnnouncementId += 1;
...
async function handleApi(req, res) {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/reveal") {
      const body = await readBody(req);
      const playerId = String(body.playerId || "");
      const result = revealAnswer(playerId);
      if (!result.ok) {
        sendJson(res, 403, result);
        return;
      }
    ...
    if (req.method === "POST" && req.url === "/api/winner") {
      const body = await readBody(req);
      const playerId = String(body.playerId || "");
      const result = announceWinner(playerId);
      if (!result.ok) {
        sendJson(res, 403, result);
        return;
      }
    ...
    if (req.method === "POST" && req.url === "/api/music") {
      ...
      if (body.musicVolume !== undefined) {
        ...
        if (Number.isFinite(volume)) game.musicVolume = Math.max(0, Math.min(1, volume));
      }
      broadcast();
      sendJson(res, 200, { ok: true, state: publicState(playerId) });
      return;
