...
const els = {
  ...
  winnerButton: document.querySelector("#winnerButton"),
...
function renderVisual(challenge, revealedAnswer) {
  ...
  if (revealedAnswer) {
    ...
    answer.textContent = revealedAnswer;
    els.visual.appendChild(answer);
  }
...
function renderOptions(game) {
  ...
  for (const option of game.options || []) {
    ...
    if (me.correctGuess && me.correctGuess === option.toLowerCase()) button.classList.add("correct");
...
function renderChallengeOptions(challenges) {
  const currentContinent = els.continentSelect.value;
  const continents = [...new Set(challenges.map(challenge => challenge.continent))];
  els.continentSelect.innerHTML = "";
  ...
  if (continents.includes(currentContinent)) {
    els.continentSelect.value = currentContinent;
  }
  populateCountryOptions(challenges);
}
...
function showWinnerOverlay(winner) {
  ...
  card.innerHTML = `<span>Winner</span><strong>${winner.name}</strong><small>${winner.score} pts</small>`;
  els.winnerOverlay.appendChild(card);

  const colors = ["#c81e1e", "#ffc94c", "#3aa7d8", "#63b75d", "#f66d57", "#7a70c9"];
  for (let index = 0; index < 90; index += 1) {
    const particle = document.createElement("i");
    particle.style.left = `${Math.random() * 100}%`;
    particle.style.background = colors[index % colors.length];
    particle.style.animationDelay = `${Math.random() * 0.9}s`;
    particle.style.animationDuration = `${2.6 + Math.random() * 2.2}s`;
    particle.style.transform = `rotate(${Math.random() * 180}deg)`;
    els.winnerOverlay.appendChild(particle);
  }

  els.winnerOverlay.classList.remove("show");
  void els.winnerOverlay.offsetWidth;
  els.winnerOverlay.classList.add("show");
  window.setTimeout(() => els.winnerOverlay.classList.remove("show"), 10000);
}
...
function render(game) {
  ...
  if (game.winnerAnnouncement
    && game.winnerAnnouncementId
    && game.winnerAnnouncementId !== state.winnerAnnouncementId) {
    state.winnerAnnouncementId = game.winnerAnnouncementId;
    showWinnerOverlay(game.winnerAnnouncement);
  } else if (!game.winnerAnnouncement) {
    state.winnerAnnouncementId = 0;
    hideWinnerOverlay();
  }
  renderVisual(game.challenge, game.revealedAnswer);
  renderOptions(game);
...
async function revealAnswer() {
  try {
    const data = await api("/api/reveal", { playerId: state.playerId });
    render(data.state);
  } catch (error) {
    els.message.textContent = error.message;
  }
...
async function announceWinner() {
  try {
    const data = await api("/api/winner", { playerId: state.playerId });
    render(data.state);
  } catch (error) {
    els.message.textContent = error.message;
  }
...
els.winnerButton.addEventListener("click", announceWinner);
